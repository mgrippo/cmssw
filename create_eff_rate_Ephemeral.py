...
def GetRate(n_initial, df_zb, deeptau_thr, pt_thr):
    ...
    rate = (float(n_events_passed.GetValue())/float(n_initial))*77560.28 #forL1
    #print("rate: %2f Hz"%(rate))
    ci = proportion_confint(count=n_events_passed.GetValue(), nobs=n_initial, alpha=0.32, method='beta')
    err_low = (rate - ci[0]*77560.28)
    err_up = (ci[1]*77560.28 - rate)
...
def BaseRate(n_initial, df_zb, pt_thr):
    ...
    rate_m = (float(n_events_passed_m.GetValue())/float(n_initial))*77560.28 #forL1
    ci_m = proportion_confint(count=n_events_passed_m.GetValue(), nobs=n_initial, alpha=0.32, method='beta')
    err_low_m = (rate_m - ci_m[0]*77560.28)
    err_up_m = (ci_m[1]*77560.28 - rate_m)
...
def BaseRate_complete(n_initial, df_zb, pt_thr):
    ...
    rate_l = (float(n_events_passed_l.GetValue())/float(n_initial))*77560.28 #forL1
    ci_l = proportion_confint(count=n_events_passed_l.GetValue(), nobs=n_initial, alpha=0.32, method='beta')
    err_low_l = (rate_l - ci_l[0]*77560.28)
    err_up_l = (ci_l[1]*77560.28 - rate_l)
    # #tight
    ...
    rate_t = (float(n_events_passed_t.GetValue())/float(n_initial))*77560.28 #forL1
    ci_t = proportion_confint(count=n_events_passed_t.GetValue(), nobs=n_initial, alpha=0.32, method='beta')
    err_low_t = (rate_t - ci_t[0]*77560.28)
    err_up_t = (ci_t[1]*77560.28 - rate_t)
