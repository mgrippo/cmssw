...
class Discriminator:
    ...
    def CountPassed(self, df, wp):
        if self.from_tuple or self.wp_column in df.columns:
            flag = 1 << wp
    ...
    def CreateRocCurve(self, df, ref_roc = None):
        ...
        wp_roc = None
        if self.raw:
            fpr, tpr, thresholds = metrics.roc_curve(df['gen_tau'].values, df[self.column].values,sample_weight=df.weight.values)
            roc = RocCurve(len(fpr), self.color, False, dashed=self.dashed)
            roc.pr[0, :] = fpr
            roc.pr[1, :] = tpr
            roc.thresholds = thresholds
            roc.auc_score = metrics.roc_auc_score(df['gen_tau'].values, df[self.column].values,sample_weight=df.weight.values)
        if n_wp > 0:
...
core_branches = [ 'tau_pt', 'lepton_gen_match', 'tau_decayMode', 'tau_looseIsoAbs', 'tau_looseIsoRel',
                  'tau_mediumIsoAbs', 'tau_mediumIsoRel', 'tau_tightIsoAbs', 'tau_tightIsoRel',
                  'deepTau_VSe', 'deepTau_VSmu', 'deepTau_VSjet']
...
all_discriminators = {
    'e': [
        Discriminator('MVA vs. electrons (JINST 13 (2018) P10005)', 'cut_based_iso', False, False, 'green',
    ...
    'mu': [
        Discriminator('Cut based (JINST 13 (2018) P10005)', 'cut_based_iso', False, False, 'green',
                      [ DiscriminatorWP.Loose, DiscriminatorWP.Medium, DiscriminatorWP.Tight] ),
...
all_discriminators['e'].append(Discriminator(deep_results_label + ' vs. electrons', 'deepTau_VSe', True, False, 'blue',
...
all_discriminators['mu'].append(Discriminator(deep_results_label + ' vs. muons', 'deepTau_VSmu', True, False, 'blue',
    [ DiscriminatorWP.VLoose, DiscriminatorWP.Loose, DiscriminatorWP.Medium, DiscriminatorWP.Tight ],
    working_points_thrs = deep_wp_thrs['mu']))
all_discriminators['jet'].append(Discriminator(deep_results_label + ' vs. jets', 'deepTau_VSjet', True, False, 'blue',
    [ DiscriminatorWP.VVVLoose, DiscriminatorWP.VVLoose, DiscriminatorWP.VLoose, DiscriminatorWP.Loose,
      DiscriminatorWP.Medium, DiscriminatorWP.Tight, DiscriminatorWP.VTight, DiscriminatorWP.VVTight ],
...
def CreateDF(file_name, filter, filter_ele, filter_mu):
    ...
    df['gen_tau'] = pandas.Series((df.lepton_gen_match == 5).values.astype(int), index=df.index)
    df['gen_ele'] = pandas.Series(((df.lepton_gen_match == 1) | (df.lepton_gen_match == 3)).values.astype(int), index=df.index)
    df['gen_mu'] = pandas.Series(((df.lepton_gen_match == 2) | (df.lepton_gen_match == 4)).values.astype(int), index=df.index)
    df['weight'] = pandas.Series(1, index=df.index)

    cut_based_iso = np.zeros(df.shape[0],dtype=int)
...
plot_setups = {
    'e': PlotSetup(ylabel='Electron mis-id probability', xlim=[0.1, 1], ratio_yscale='log',
                   ylim=[2e-5, 1.5], ratio_ylim=[0.5, 40]),
    'mu': PlotSetup(ylabel='Muon mis-id probability', xlim=[0.1, 1], ratio_yscale='log',
                    ylim=[2e-5, 1.5],
                    ratio_ylim=[ [0.5, 20], [0.5, 20], [0.5, 20], [0.5, 20], [0.5, 20],
                                 [0.5, 20], [0.5, 20], [0.5, 50], [0.5, 50], [0.5, 50] ] ),
    'jet': PlotSetup(ylabel='Jet mis-id probability', ratio_ylable_pad=30, xlim=[0.0, 1],
    ...
    for pt_index in range(len(pt_bins) - 1):
            ...
            if rocs[n].auc_score is not None:
                ...
                #thrs = [ find_threshold(rocs[n].pr[1, :], rocs[n].thresholds, pr) for pr in target_prs ]
                 print('[{}, {}] {} roc_auc = {}'.format(pt_bins[pt_index], pt_bins[pt_index + 1], names[n],
                                                         rocs[n].auc_score))
                #print(' {} roc_auc = {}'.format(names[n],rocs[n].auc_score))
                #print(thrs)
        ...
        if public_plots:
            ...
            ax.text(0.12, header_y, 'Simulation Preliminary', fontsize=14, transform=ax.transAxes, fontstyle='italic',
                    fontfamily='sans-serif')
            ax.text(0.73, header_y, '2018 (13 TeV)', fontsize=13, transform=ax.transAxes, fontweight='bold',
                    fontfamily='sans-serif')
        else:
