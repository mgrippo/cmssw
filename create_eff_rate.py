...
def GetRate(n_initial, df_zb, deeptau_thr, pt_thr):
    ...
    df_zb = df_zb.Define('ntaus','getNtaus(tau_decayModeFindingNewDMs,deepTau_VSjet,tau_pt,{},{})'.format(deeptau_thr,pt_thr))
...
def GetEfficiency(n_initial,df_vbf, deeptau_thr, pt_thr,pt_bin_1,pt_bin_2):
    ...
    df_vbf = df_vbf.Define('ntaus','getNtaus_inPtBin(tau_decayModeFindingNewDMs,deepTau_VSjet,tau_pt,{},{},{},{})'.format(deeptau_thr,pt_thr,pt_bin_1,pt_bin_2))
...
def GetEfficiency_Simple(n_initial,df_vbf, deeptau_thr, pt_thr):
    ...
    # df_vbf_den = df_vbf.Filter('ntaus_den >= 2')#to bechanged to 2!!!
    # n_events = df_vbf_den.Count()
    df_vbf = df_vbf.Define('ntaus','getNtaus(tau_decayModeFindingNewDMs,deepTau_VSjet,tau_pt,{},{})'.format(deeptau_thr,pt_thr))
    df_vbf_num = df_vbf.Filter('ntaus >= 2')#to bechanged to 2!!!
    n_events_passed = df_vbf_num.Count()
    eff = (float(n_events_passed.GetValue())/float(n_initial))
    ci = proportion_confint(count=n_events_passed.GetValue(), nobs=n_initial, alpha=0.32, method='beta')
...
def BaseRate(n_initial, df_zb, pt_thr):
    n_events = df_zb.Count()
    #medium
    df_zb_m = df_zb.Define('ntaus','getNtaus_base(tau_decayModeFindingNewDMs,tau_mediumIsoAbs,tau_mediumIsoRel,tau_pt,{})'.format(pt_thr))
...
def BaseRate_complete(n_initial, df_zb, pt_thr):
    ...
    err_up_m = (ci_m[1]*2544*11245 - rate_m)
    #loose
    df_zb_l = df_zb.Define('ntaus_l','getNtaus_base_loose(tau_decayModeFindingNewDMs,tau_looseIsoAbs,tau_looseIsoRel,tau_pt,{})'.format(pt_thr))
    ...
    # #tight
    df_zb_t = df_zb.Define('ntaus_t','getNtaus_base_tight(tau_decayModeFindingNewDMs,tau_tightIsoAbs,tau_tightIsoRel,tau_pt,{})'.format(pt_thr))
...
def BaseEfficiency_Simple(n_initial,df_vbf,pt_thr):
    ...
    # n_events = df_vbf_den.Count()
    #medium
    df_vbf_m = df_vbf.Define('ntaus_m','getNtaus_base(tau_decayModeFindingNewDMs,tau_mediumIsoAbs,tau_mediumIsoRel,tau_pt,{})'.format(pt_thr))
    ...
    #loose
    df_vbf_l = df_vbf.Define('ntaus_l','getNtaus_base_loose(tau_decayModeFindingNewDMs,tau_looseIsoAbs,tau_looseIsoRel,tau_pt,{})'.format(pt_thr))
    ...
    #tight
    df_vbf_t = df_vbf.Define('ntaus_t','getNtaus_base_tight(tau_decayModeFindingNewDMs,tau_tightIsoAbs,tau_tightIsoRel,tau_pt,{})'.format(pt_thr))
...
def BaseEfficiency(n_initial,df_vbf,pt_thr,pt_bin_1,pt_bin_2):
    ...
    # n_events = df_vbf_den.Count()
    #medium
    df_vbf_m = df_vbf.Define('ntaus_m','getNtaus_base_inPtBin_Medium(tau_decayModeFindingNewDMs,tau_mediumIsoAbs,tau_mediumIsoRel,tau_pt,{},{},{})'.format(pt_thr,pt_bin_1,pt_bin_2))
    df_vbf_m = df_vbf_m.Filter('ntaus_m >= 2')#to bechanged to 2!!!
    n_events_passed_m = df_vbf_m.Count()
    eff_m = (float(n_events_passed_m.GetValue())/float(n_initial))
    ci_m = proportion_confint(count=n_events_passed_m.GetValue(), nobs=n_initial, alpha=0.32, method='beta')
    ...
    err_up_m = (ci_m[1] - eff_m)
    #loose
    df_vbf_l = df_vbf.Define('ntaus_l','getNtaus_base_inPtBin_Loose(tau_decayModeFindingNewDMs,tau_looseIsoAbs,tau_looseIsoRel,tau_pt,{},{},{})'.format(pt_thr,pt_bin_1,pt_bin_2))
    df_vbf_l = df_vbf_l.Filter('ntaus_l >= 2')#to bechanged to 2!!!
    n_events_passed_l = df_vbf_l.Count()
    eff_l = (float(n_events_passed_l.GetValue())/float(n_initial))
    ci_l = proportion_confint(count=n_events_passed_l.GetValue(), nobs=n_initial, alpha=0.32, method='beta')
    ...
    err_up_l = (ci_l[1] - eff_l)
    #tight
    df_vbf_t = df_vbf.Define('ntaus_t','getNtaus_base_inPtBin_Tight(tau_decayModeFindingNewDMs,tau_tightIsoAbs,tau_tightIsoRel,tau_pt,{},{},{})'.format(pt_thr,pt_bin_1,pt_bin_2))
    df_vbf_t = df_vbf_t.Filter('ntaus_t >= 2')#to bechanged to 2!!!
    n_events_passed_t = df_vbf_t.Count()
    eff_t = (float(n_events_passed_t.GetValue())/float(n_initial))
    ci_t = proportion_confint(count=n_events_passed_t.GetValue(), nobs=n_initial, alpha=0.32, method='beta')
...
if not tfile.IsZombie() and not tfile_rate.IsZombie():
    ...
    afterL1Counter_eff = f_eff.Get("afterL1Filter_counter")
    intermidiateCounter_eff = f_eff.Get("intermidiate_counter")

    df_eff = df_eff.Define('ntaus_den','getNtaus(tau_decayModeFindingNewDMs,deepTau_VSjet,tau_pt,{},{})'.format(0,0))
    df_eff_den = df_eff.Filter('ntaus_den >= 2')#to bechanged to 2!!!
    n_events_in_df = df_eff_den.Count()

    print("Efficiency steps in VBF")
    print("After L1",float(afterL1Counter_eff.GetEntries())/float(initialCounter_eff.GetEntries()))
    print("After HLT tau reco before Iso",float(intermidiateCounter_eff.GetEntries())/float(initialCounter_eff.GetEntries()))
    print("After HLT tau reco after Iso",float(n_events_in_df.GetValue())/float(initialCounter_eff.GetEntries()))
    ...
    for thr_index in range(len(pt_thrs)):
        ...
        ax[thr_index].set_title('Rate vs efficiency for '+str(pt_thrs[thr_index])+' GeV')
    ...
    # fig2.savefig("eff_vs_pt_cutbased.pdf")
        fig[thr_index].savefig('eff_vs_rate_thr_'+str(pt_thrs[thr_index])+'_newDM.pdf')
    ...
    fig_eff.savefig('eff_vs_pt_thr_newDM.pdf')
